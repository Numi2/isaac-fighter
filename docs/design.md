...
The environment never emits scripted attack primitives. A policy controls normalized joint-position offsets for its own robot. The combat objective is expressed through rewards and fight rules, while the policy decides gait, stance, contact timing, pushing, recovery, retreat, and engagement.
...
The active training run writes skrl checkpoints into a persistent policy pool. The pool records policy id, version, Elo, win/loss/draw rates, checkpoint path, and tags. The opponent sampler combines Elo proximity, observed weakness, and recency. A historical-opponent wrapper can replace one side's actions with a sampled learned TorchScript policy. When the pool is empty, the wrapper leaves actions unchanged so the first IPPO/MAPPO population can bootstrap.
